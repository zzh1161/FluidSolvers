...
if args.demo == 0:
    ...
    solver_b.init_velocity(leapfrog.velx, leapfrog.vely)
    solver_m.init_velocity(leapfrog.velx, leapfrog.vely)
...
while cur_frame < total_frame:
    ...
    if args.scheme == 0:
        ...
        solver_b.advance()
        gui.set_image(solver_b.abs_vor)
    elif args.scheme == 3:
        solver_m.advance()
...
#     # subplot 1
#     im1 = axs[0, 0].imshow(np.rot90(solver_1.abs_vor.to_numpy()), cmap='jet', vmin=vmin, vmax=vmax)
#     axs[0, 0].set_title('1st order Semi-Lagrangian')
#     axs[0, 0].axis('off')
#     # subplot 2
#     axs[0, 1].imshow(np.rot90(solver_2.abs_vor.to_numpy()), cmap='jet', vmin=vmin, vmax=vmax)
#     axs[0, 1].set_title('2nd order Semi-Lagrangian')
#     axs[0, 1].axis('off')
#     # subplot 3
#     axs[1, 0].imshow(np.rot90(solver_b.abs_vor.to_numpy()), cmap='jet', vmin=vmin, vmax=vmax)
#     axs[1, 0].set_title('CF+BFECC')
#     axs[1, 0].axis('off')
#     # subplot 4
#     axs[1, 1].imshow(np.rot90(solver_m.abs_vor.to_numpy()), cmap='jet', vmin=vmin, vmax=vmax)
